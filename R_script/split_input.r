...
data_in <- data.frame(fread(file=input_param[1]))
...
for(i in 1:cut_size){
write.table(data.frame(gene_id=data_in[,1],data_in[,which(sub_idx==i)+1]),file=paste0("./BIRD_tmp/BIRD_input_part",i),row.names=FALSE,col.names=TRUE,sep="\t",quote=FALSE)
}
